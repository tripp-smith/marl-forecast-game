...
_predictit_rate_limiter = RateLimiter(calls_per_second=5)


@dataclass(frozen=True)
class PredictItAdapter:
    """Fetches prediction-market prices from the PredictIt JSON API, with synthetic fallback."""
    ...
    def _fetch_api(self, periods: int) -> list[NormalizedRecord]:
        ...
        _predictit_rate_limiter.acquire()
    ...
    def fetch(self, periods: int = 30) -> list[NormalizedRecord]:
        """Return up to *periods* market-price records from PredictIt or synthetic data."""
        try:
